...
public class MessageA08p03pr extends MessageCommon {
	
	public MessageA08p03pr() {
		super();
		// TODO Auto-generated constructor stub
	}

	public MessageA08p03pr(int pERSON_SERDOC, int pERSON_NUMDOC,
	...
	protected void createMiddle(int count, Namespace namespace,
		...
		for (int i = 1; i < count; i++) {
			...
			in1_1_16.addContent(new Element("XPN.1", namespace).addContent(new Element("FN.1", namespace).addContent(dataList.get(i).get(LAST_FAM))));
			in1_1_16.addContent(new Element("XPN.2", namespace).addContent(dataList.get(i).get(LAST_IM)));
			in1_1_16.addContent(new Element("XPN.3", namespace).addContent(
					(dataList.get(i).get(LAST_OT).equals("мер") || dataList.get(i).get(LAST_OT).equals("-")) ? "" : dataList.get(i).get(LAST_OT)));
	
			in1_1.addContent(new Element("IN1.18", namespace).addContent(dataList.get(i).get(LAST_DR)));
			...
			System.out.println("pfr_snils:" + dataList.get(i).get(PFR_SNILS));
			//IN1-2
			...
			if ("".equals(dataList.get(i).get(VS_NUM)) || "NULL".equals(dataList.get(i).get(VS_NUM))) {
				if(dataList.get(i).get(TYPE_POL).equals("щ")){
					in1_2.addContent(new Element("IN1.35", namespace).addContent("щ"));
