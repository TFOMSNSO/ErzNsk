...
public class MessageZp1pr extends MessageCommon {
	...
	public MessageZp1pr(int pERSON_SERDOC, int pERSON_NUMDOC,
	...
	protected void createMiddle(int count, Namespace namespace,	Element rootElement, String curDate) {
		...
		for (int i = 1; i < count; i++) {
			...
			.addContent(new Element("FN.1", namespace).addContent(dataList.get(i).get(LAST_FAM))));
			qpd6.addContent(new Element("XPN.2", namespace).addContent(dataList.get(i).get(LAST_IM)));
			qpd6.addContent(new Element("XPN.3", namespace).addContent(
					(dataList.get(i).get(LAST_OT).equals("ÍÅÒ") || dataList.get(i).get(LAST_OT).equals("-")) ? "" : dataList.get(i).get(LAST_OT)));
			
			qpd.addContent(new Element("QPD.7", namespace).addContent(dataList.get(i).get(LAST_DR)));
