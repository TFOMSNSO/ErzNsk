...
public class MessageA08p03For extends MessageCommon {
	
	public MessageA08p03For() {
		super();
		// TODO Auto-generated constructor stub
	}

	public MessageA08p03For(int pERSON_SERDOC, int pERSON_NUMDOC,
			...
			Element rootElement, String curDate)
	{
		for (int i = 1; i < count; i++) 
		{
			...
			if(dataList.get(i).get(D_12_PLUS1) == null || "".equals(dataList.get(i).get(D_12_PLUS1)) || dataList.get(i).get(D_12_PLUS1).length() < 10) {
				in1_2.addContent(new Element("IN1.12", namespace).addContent(""));
			} else {
				in1_2.addContent(new Element("IN1.12", namespace).addContent(dataList.get(i).get(D_12_PLUS1).substring(0, 10)));
			}
			in1_2.addContent(new Element("IN1.13", namespace).addContent(dataList.get(i).get(D2)));
			in1_2.addContent(new Element("IN1.15", namespace).addContent(dataList.get(i).get(OKATO_3)));
			in1_2.addContent(new Element("IN1.35", namespace).addContent(dataList.get(i).get(TYPE_POL)));
			if("¹".equals(dataList.get(i).get(POL))) {
				in1_2.addContent(new Element("IN1.36", namespace).addContent(("".equals(dataList.get(i).get(ENP_PA)) ? dataList.get(i).get(ENP) : dataList.get(i).get(ENP_PA))));
			} else {
				in1_2.addContent(new Element("IN1.36", namespace).addContent(dataList.get(i).get(POL)));
			}
