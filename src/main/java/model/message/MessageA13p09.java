...
public class MessageA13p09 extends MessageCommon {
	
	public MessageA13p09() {
		super();
		// TODO Auto-generated constructor stub
	}

	public MessageA13p09(int pERSON_SERDOC, int pERSON_NUMDOC,
	...
	protected void createMiddle(int count, Namespace namespace,	Element rootElement, String curDate) {
		...
		for (int i = 1; i < count; i++) {
			...
			RandomGUID guidMsh = createMsh(namespace, curDate, adt_a01, "ADT", "A13", "ADT_A01");	
				
			Element evn = new Element("EVN", namespace);
			adt_a01.addContent(evn);
			evn.addContent(new Element("EVN.2", namespace).addContent(curDate));
			evn.addContent(new Element("EVN.4", namespace).addContent("Ï09"));
